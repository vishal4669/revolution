...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.slotBooking.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.slot-bookings.update", [$slotBooking->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $slotBooking->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('hrs_used') ? 'is-invalid' : '' }}" type="text" name="hrs_used" id="hrs_used" value="{{ old('hrs_used', $slotBooking->hrs_used) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('date') ? 'is-invalid' : '' }}" type="text" name="date" id="date" value="{{ old('date', $slotBooking->date) }}" required>
                @if($errors->has('date'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('start_time') ? 'is-invalid' : '' }}" type="text" name="start_time" id="start_time" value="{{ old('start_time', $slotBooking->start_time) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('end_time') ? 'is-invalid' : '' }}" type="text" name="end_time" id="end_time" value="{{ old('end_time', $slotBooking->end_time) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('booked_via') ? 'is-invalid' : '' }}" name="booked_via" id="booked_via">
                    ...
                    @foreach(App\Models\SlotBooking::BOOKED_VIA_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('booked_via', $slotBooking->booked_via) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <label>{{ trans('cruds.slotBooking.fields.is_cancelled') }}</label>
                @foreach(App\Models\SlotBooking::IS_CANCELLED_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('is_cancelled') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="is_cancelled_{{ $key }}" name="is_cancelled" value="{{ $key }}" {{ old('is_cancelled', $slotBooking->is_cancelled) === (string) $key ? 'checked' : '' }}>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.slotBooking.fields.is_cancelled_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="cancelled_by">{{ trans('cruds.slotBooking.fields.cancelled_by') }}</label>
                <input class="form-control {{ $errors->has('cancelled_by') ? 'is-invalid' : '' }}" type="text" name="cancelled_by" id="cancelled_by" value="{{ old('cancelled_by', $slotBooking->cancelled_by) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('remarks') ? 'is-invalid' : '' }}" type="text" name="remarks" id="remarks" value="{{ old('remarks', $slotBooking->remarks) }}">
