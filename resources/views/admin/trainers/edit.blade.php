    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.trainers.update", [$trainer->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $trainer->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('trainer_cost') ? 'is-invalid' : '' }}" type="number" name="trainer_cost" id="trainer_cost" value="{{ old('trainer_cost', $trainer->trainer_cost) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{!! old('description', $trainer->description) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type" id="type" required>
                    ...
                    @foreach(App\Models\Trainer::TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('type', $trainer->type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('type'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('serial_number') ? 'is-invalid' : '' }}" type="text" name="serial_number" id="serial_number" value="{{ old('serial_number', $trainer->serial_number) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('rent_month') ? 'is-invalid' : '' }}" type="number" name="rent_month" id="rent_month" value="{{ old('rent_month', $trainer->rent_month) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('rent_hour') ? 'is-invalid' : '' }}" type="number" name="rent_hour" id="rent_hour" value="{{ old('rent_hour', $trainer->rent_hour) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Trainer::IS_ACTIVE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('is_active') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="is_active_{{ $key }}" name="is_active" value="{{ $key }}" {{ old('is_active', $trainer->is_active) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Trainer::IS_RENTED_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('is_rented') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="is_rented_{{ $key }}" name="is_rented" value="{{ $key }}" {{ old('is_rented', $trainer->is_rented) === (string) $key ? 'checked' : '' }} required>
