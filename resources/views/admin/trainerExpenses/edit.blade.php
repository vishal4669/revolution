...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.trainerExpense.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.trainer-expenses.update", [$trainerExpense->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="repair_date">{{ trans('cruds.trainerExpense.fields.repair_date') }}</label>
                <input class="form-control date {{ $errors->has('repair_date') ? 'is-invalid' : '' }}" type="text" name="repair_date" id="repair_date" value="{{ old('repair_date', $trainerExpense->repair_date) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('trainer') ? 'is-invalid' : '' }}" name="trainer_id" id="trainer_id" required>
                    @foreach($trainers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('trainer_id') ? old('trainer_id') : $trainerExpense->trainer->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('amount') ? 'is-invalid' : '' }}" type="number" name="amount" id="amount" value="{{ old('amount', $trainerExpense->amount) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{!! old('description', $trainerExpense->description) !!}</textarea>
          ...
          return loader.file
            .then(function (file) {
              return new Promise(function(resolve, reject) {
                ...
                xhr.open('POST', '{{ route('admin.admin.trainer-expenses.storeCKEditorImages') }}', true);
