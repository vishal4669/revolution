...
@section('content')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.cycles.create') }}">
                {{ trans('global.add') }} {{ trans('cruds.cycle.title_singular') }}
            </a>
        </div>
    </div>
<div class="card">
    ...
    <div class="card-body">
    <table class=" table table-bordered table-striped table-hover ajaxTable datatable datatable-Cycle">
            <thead>
                <tr>
                    <th>
                       Actions
                ...
                <tr>
                    ...
                    <td>
                        <select class="search" strict="true">
                            ...
                            @foreach(App\Models\Cycle::TYPE_SELECT as $key => $item)
                    ...
                    <td>
                        <select class="search" strict="true">
                            <option value>{{ trans('global.all') }}</option>
                            @foreach(App\Models\Cycle::IS_ACTIVE_RADIO as $key => $item)
                                <option value="{{ $key }}">{{ $item }}</option>
                            @endforeach
                        </select>
                    </td>
                    <td>
                        <select class="search" strict="true">
                            <option value>{{ trans('global.all') }}</option>
                            @foreach(App\Models\Cycle::IS_RENTED_RADIO as $key => $item)
                                <option value="{{ $key }}">{{ $item }}</option>
                            @endforeach
                        </select>
                    </td>
...
@can('cycle_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.cycles.massDestroy') }}",
...
@endcan
  ...
  let dtOverrideGlobals = {
    ...
    columns: [        
        ...
        { data: 'cycle_cost', name: 'cycle_cost' },
        { data: 'type', name: 'type' },
  ...
  let table = $('.datatable-Cycle').DataTable(dtOverrideGlobals);
